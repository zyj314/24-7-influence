...
class CFEMarket:
    ...
    def create_model(self, scenario='volumetric', matching_target=0.90):
        ...
        m.L = Set(initialize=self.branch_num_idx)
        ...
        PTDF = lambda l, b: self.PTDF_matrix[b][self.branch_idx[l]]
        ...
        m.dc_flow = Constraint(m.L, m.T, rule=lambda m, l, t: m.Pf[self.branch_idx[l], t] == sum(
            (sum(m.P_gen[g, t] * BusGen(g, b) for g in m.G)
             - self.Pd.loc[b, t] + self.solar_output.loc[b, t] + self.wind_output.loc[b, t]) * PTDF(l, b) for b in
            m.B) + sum((m.P_discharge[s, t] - m.P_charge[s, t]) * PTDF(l, s) for s in m.S))
        m.branch_upper = Constraint(m.L, m.T, rule=lambda m, l, t: m.Pf[self.branch_idx[l], t] <= self.Pf_max.at[
            self.branch_idx[l], 'Pf_max'])
        m.branch_lower = Constraint(m.L, m.T, rule=lambda m, l, t:  - self.Pf_max.at[
            self.branch_idx[l], 'Pf_max'] <= m.Pf[self.branch_idx[l], t])
        ...
        if scenario == 'volumetric':
            m.matching = Constraint(rule=lambda m:  sum(sum(self.participant_demand.loc[b, t] for b in m.B) for t in m.T) <= \
                sum(sum(self.solar_output.loc[b, t] + self.wind_output.loc[b, t] for b in m.B) + 
                    sum(m.P_discharge[s, t] - m.P_charge[s, t] for s in m.S) 
                for t in m.T))
            ...
            m.matching = Constraint(m.T, rule=lambda m, t:  
                ...
                sum(self.solar_output.loc[b, t] + self.wind_output.loc[b, t] for b in m.B) + \
                sum(m.P_discharge[s, t] - m.P_charge[s, t] for s in m.S))
        ...
        for t in m.T:
            ...
            for b in m.B:
                ...
                congestion_component = sum(
                    (m.dual[m.branch_lower[l, t]] - m.dual[m.branch_upper[l, t]]) *
                    self.PTDF_matrix[b][self.branch_idx[l]]
                    for l in self.branch_num_idx
                )
...
def validate_cost_timeseries(market, scenario='volumetric', delta=1e-2):
...
if __name__ == "__main__":
    ...
    #  Volumetric 场景
    validate_cost_timeseries(market, scenario='volumetric', delta=1e-2)

    #  Hourly 场景：
    validate_cost_timeseries(market, scenario='hourly', delta=1e-2)
